...
## 4. Kafka  
카프카는 대규모로 발생하는 작은 데이터이 있다. 초당 수십, 수백, 수천건이 발생하는 데이터 이 데이터들을 다 동기화 처리하기에는 무리가 있기 때문에, 이를 비동기방식으로 처리하게 된다. 비동기 처리를 하기 위해서 큐(Queue)를 사용하게 되고, 이 큐가 버퍼의 역할을 수행하게 된다. 그래서 카프카는 버퍼처리와 트랜잭션 처리를 수행한다. 이러한 수행으로 인해, 카프카는 대규모 메세지 처리를 빠르고 안정적으로 수행한다. 카프카의 주요 구성요소는 다음과 같다.
